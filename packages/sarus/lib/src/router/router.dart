...
final _removeBody = createMiddleware(
  responseHandler: (r) {
...
class Router {
  ...
  void add(HttpMethod verb, String route, Function handler) {
    if (verb == HttpMethod.get) {
      ...
      _routes.add(
        RouterEntry(HttpMethod.head, route, handler, middleware: _removeBody),
      );
    }
  ...
  void all(String route, Function handler) {
    _routes.add(RouterEntry(HttpMethod.all, route, handler));
  }
  ...
  Future<Response> call(Request request) async {
    ...
    for (final route in _routes) {
      if (route.httpMethod.name != request.method.toUpperCase() &&
          route.httpMethod != HttpMethod.all) {
        continue;
      }
