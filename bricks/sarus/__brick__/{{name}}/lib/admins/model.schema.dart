...
class _AdminModelRepository extends BaseRepository
  ...
  Future<List<int>> insert(List<AdminModelInsertRequest> requests) async {
    ...
    var rows = await db.execute(
      Sql.named(
        'INSERT INTO "admins" ( "first_name", "last_name", "password", "email", "role", "status", "created_at", "updated_at" )\n'
        'VALUES ${requests.map((r) => '( ${values.add(r.firstName)}:text, ${values.add(r.lastName)}:text, ${values.add(r.password)}:text, ${values.add(r.email)}:text, ${values.add(r.role)}:text, ${values.add(r.status)}:boolean, ${values.add(r.createdAt)}:text, ${values.add(r.updatedAt)}:text )').join(', ')}\n'
        'RETURNING "id"',
      ),
  ...
  Future<void> update(List<AdminModelUpdateRequest> requests) async {
    ...
    await db.execute(
      Sql.named(
        'UPDATE "admins"\n'
        'SET "first_name" = COALESCE(UPDATED."first_name", "admins"."first_name"), "last_name" = COALESCE(UPDATED."last_name", "admins"."last_name"), "password" = COALESCE(UPDATED."password", "admins"."password"), "email" = COALESCE(UPDATED."email", "admins"."email"), "role" = COALESCE(UPDATED."role", "admins"."role"), "status" = COALESCE(UPDATED."status", "admins"."status"), "created_at" = COALESCE(UPDATED."created_at", "admins"."created_at"), "updated_at" = COALESCE(UPDATED."updated_at", "admins"."updated_at")\n'
        'FROM ( VALUES ${requests.map((r) => '( ${values.add(r.id)}:int8::int8, ${values.add(r.firstName)}:text::text, ${values.add(r.lastName)}:text::text, ${values.add(r.password)}:text::text, ${values.add(r.email)}:text::text, ${values.add(r.role)}:text::text, ${values.add(r.status)}:boolean::boolean, ${values.add(r.createdAt)}:text::text, ${values.add(r.updatedAt)}:text::text )').join(', ')} )\n'
        'AS UPDATED("id", "first_name", "last_name", "password", "email", "role", "status", "created_at", "updated_at")\n'
